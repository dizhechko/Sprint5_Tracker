...
func (r Running) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var inform InfoMessage
	inform.TrainingType = r.TrainingType
	inform.Duration = r.Duration
	inform.Distance = r.distance()
	inform.Speed = r.meanSpeed()
	inform.Calories = r.Calories()
...
type Walking struct {
	// добавьте необходимые поля в структуру
	Training
	Height float64 // рост пользователя НАДО ПЕРЕВЕСТИ В МЕТРЫ
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Training.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/height)*CaloriesSpeedHeightMultiplier*w.Training.Weight) * w.Training.Duration.Hours() * MinInHours)
}
...
func (w Walking) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var inform InfoMessage
	inform.TrainingType = w.TrainingType
	inform.Duration = w.Duration
	inform.Distance = w.distance()
	inform.Speed = w.meanSpeed()
	inform.Calories = w.Calories()
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var inform InfoMessage
	inform.TrainingType = s.TrainingType
	inform.Duration = s.Duration
	inform.Distance = s.distance()
	inform.Speed = s.meanSpeed()
	inform.Calories = s.Calories()
